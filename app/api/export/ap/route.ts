...
export async function GET() {
  ...
  const exportRows = rows.map((row) => ({
    ...
    거래처명: row.client_nm ?? "",
    항목: row.item_type ?? "",
    세부매입내용: row.description ?? "",
    금액: Number(row.amount),
    검수상태: normalizeReviewStatus(row.pay_status),
    지급상태: row.pay_status ?? "",
    원천ID: row.source_id ?? "",
  ...
  XLSX.utils.book_append_sheet(workbook, sheet, "AP 전체");
  const buffer = XLSX.write(workbook, { type: "buffer", bookType: "xlsx", cellStyles: true });

  const filename = `매입관리_전체_${yyyymmdd()}.xlsx`;
