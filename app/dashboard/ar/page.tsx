...
type ArManagementPageProps = {
  searchParams?: {
    year?: string;
  };
};

export default async function ArManagementPage({ searchParams }: ArManagementPageProps) {
  ...
  const rows: GridRowItem[] = arRows.map((row) => ({
    ...
    inspect_status: normalizeReviewStatus(row.claim_status),
